...
from .trainers.dynamic_topk import AutoEncoderDynamicTopK
...
def load_dictionary(base_path: str, device: str) -> tuple:
    ...
    elif dict_class == "AutoEncoderDynamicTopK":
        base_k = config["trainer"]["base_k"]
        min_k = config["trainer"]["min_k"]
        max_k = config["trainer"]["max_k"]
        dictionary = AutoEncoderDynamicTopK.from_pretrained(
            ae_path, base_k=base_k, min_k=min_k, max_k=max_k, device=device
        )
    else:

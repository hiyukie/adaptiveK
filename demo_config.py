...
from dictionary_learning.trainers.dynamic_topk import DynamicTopKTrainer, AutoEncoderDynamicTopK
...
from dictionary_learning.trainers.dynamic_topk import AutoEncoderDynamicTopK 
...
# dynamic SAE
num_tokens = 256_000_000
# baseline SAEs
# num_tokens = 250_000

print(f"NOTE: Training on {num_tokens} tokens")

eval_num_inputs = 10000
random_seeds = [0]
dictionary_widths = [2**14]

WARMUP_STEPS = 10
SPARSITY_WARMUP_STEPS = 15
...
#     standard=[2.4, 3.6, 4.8, 8, 12, 16],
#     standard_new=[2.4, 3.6, 4.8, 8, 12, 16],
#     p_anneal=[1.2, 1.8, 2.4, 4, 6, 8],
#     gated=[2.4, 3.6, 4.8, 8, 12, 16],
# )

SPARSITY_PENALTIES = SparsityPenalties(
    standard=[0.6, 0.9, 1.2, 2, 3, 4],
    standard_new=[0.6, 0.9, 1.2, 2, 3, 4],
    p_anneal=[0.3, 0.45, 0.6, 1, 1.5, 2],
    gated=[0.6, 0.9, 1.2, 2, 3, 4],
)
...
class PAnnealTrainerConfig(BaseTrainerConfig):
    ...
    sparsity_queue_length: int = 5
    n_sparsity_updates: int = 5
...
@dataclass
class DynamicTopKTrainerConfig(BaseTrainerConfig):
    dict_size: int
    seed: int
    sae_lr: float
    probe_lr: float
...
def get_trainer_configs(
    ...
    if TrainerType.DYNAMIC_TOP_K.value in architectures:
        ...
        context_count = 250000
        desired_epochs = 15
        batch_size = 2048
        dynamic_steps = (context_count * desired_epochs) // batch_size

        dynamic_decay_start = int(dynamic_steps * DECAY_START_FRACTION)
        dynamic_warmup_steps = 15
        # dynamic_sparsity_warmup_steps = 250

        dynamic_base_config = dict(base_config)
        dynamic_base_config["steps"] = dynamic_steps
        dynamic_base_config["decay_start"] = dynamic_decay_start
        dynamic_base_config["warmup_steps"] = dynamic_warmup_steps
        # dynamic_base_config["sparsity_warmup_steps"] = dynamic_sparsity_warmup_steps

        for seed, dict_size, learning_rate in itertools.product(
            seeds, dict_sizes, learning_rates
        ): 
            config = DynamicTopKTrainerConfig(
                **dynamic_base_config,
                trainer=DynamicTopKTrainer,
                dict_class=AutoEncoderDynamicTopK,
                sae_lr=learning_rate,
                ...
                max_k=320,
                auxk_alpha=1/32,
                wandb_name=f"DynamicTopKTrainer-{model_name}-{submodule_name}",
            )
            trainer_configs.append(asdict(config))
